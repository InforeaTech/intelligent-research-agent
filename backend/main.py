...
def research_profile(request: ProfileRequest):
    ...
    Implements cascading cache:
    - Level 1: Complete result (name+company → profile) - skips both Serper and LLM
    - Level 2: Search results (query → search data) - skips only Serper
    - Level 3: Profile generation (search data → profile) - skips only LLM
    """
    try:
        ...
        research_data = agent.gather_info(request.name, request.company, request.additional_info, serper_key, bypass_cache=request.bypass_cache)
